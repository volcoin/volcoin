...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVolCoinAmount"))
        settings.setValue("nAnonymizeVolCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeVolCoinAmount = settings.value("nAnonymizeVolCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeVolCoinAmount"))
        SoftSetArg("-anonymizeVolCoinamount", settings.value("nAnonymizeVolCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeVolCoinAmount:
            nAnonymizeVolCoinAmount = value.toInt();
            settings.setValue("nAnonymizeVolCoinAmount", nAnonymizeVolCoinAmount);
            emit anonymizeVolCoinAmountChanged(nAnonymizeVolCoinAmount);
            break;
        default:
            break;
        }
