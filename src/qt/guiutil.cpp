...
{
    // Convert VolCoin:// to VolCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("VolCoin://"))
    {
        uri.replace(0, 12, "VolCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "VolCoin.lnk";
}
...
{
    return GetAutostartDir() / "VolCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("VolCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  VolCoin-qt [" + tr("command-line options") + "]                     " + "\n";
