...
int nAnonymizeVolCoinAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VolCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VolCoin
    // Mac: ~/Library/Application Support/VolCoin
    // Unix: ~/.VolCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VolCoin";
#else
    fs::path pathRet;
...
void createConf()       //Automatic VolCoin.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "VolCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "volcoind.pid"));
