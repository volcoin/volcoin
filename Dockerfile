##################################  Notes  ##################################
# build:
#   docker build --no-cache -t volcoin .
#
# run:
#   docker run -it -p 15015:15015 volcoin
#
# run with a mounted directory for ~/.VolCoin:
#   docker run -it -p 15015:15015 -v /path/to/a/local/directory:/root/.VolCoin volcoin
#
# run will exec you into docker /bin/bash
# from there, you can run:
# volcoind # starts the volcoin deamon
#
# For accessing the volcoin JSON-RPC api from the host:
# 1. Expose RPC port in when running docker
#    docker run -it -p 15015:15015 -p 5000:XXX -v /path/to/a/local/directory:/root/.VolCoin volcoin # Replace XXX with set rpcPort in volcoin.conf
...
RUN git clone https://github.com/VolCoin/VolCoin.git VolCoin && cd VolCoin/src/ && make -f makefile.unix && cp volcoind /usr/local/sbin/volcoind

RUN mkdir /root/.VolCoin/
